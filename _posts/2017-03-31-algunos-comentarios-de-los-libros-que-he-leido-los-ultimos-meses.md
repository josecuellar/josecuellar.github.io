...
The *Domain Driven Design* approach and its strategic patterns already introduced the division of the domain into the so-called [Bounded Context](/domain-driven-design-episodio-i-empezando/) and how they collaborate with each other through the [Context Mappings](/domain-driven-design-episodio-ii-context-maps/). The proposal of a *microservices* architecture is based on the greater division of responsibilities and developing a set of small microservices that will support a distributed system.
...
In the first part of the book, it focuses on explaining the strategic patterns of DDD to deepen in later chapters on more tactical topics such as [CQRS](/domain-driven-design-episodio-iii-arquitectura/), patterns that will help us in the development such as [Command (processor) pattern](https://en.wikipedia.org/wiki/Command_pattern), [Double Dispatch pattern](https://en.wikipedia.org/wiki/Double_dispatch), [Memento pattern](https://es.wikipedia.org/wiki/Memento_(patr%C3event-store/), reconstruction of aggregates.
