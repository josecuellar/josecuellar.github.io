...
Cada **Bounded Context** contiene o dispone de la tecnología, lenguaje de programación y arquitectura mínima necesaria para satisfacer los requerimientos de negocio de su contexto mediante un **modelo de dominio**. 

Las relaciones existentes entre *Bounded Contexts* se denominan **Context Maps** y las reglas necesarias para *mapear/traducir* un modelo de dominio de un determinado contexto a otro:

**Translation Map**. Una vez identificados los *Bounded Contexts*, deberemos establecer según necesidades de cada caso el tipo de relación entre ellos, marcando la dirección de *flujo de información* mediante ***U**pstream* (exposición de servicios) y ***D**ownstream* (consumidor de servicios):
...
##### Fuente: [Taming Complex Domains with Domain Driven Design](http://www.slideshare.net/ziobrando/taming-complex-domains-with-domain-driven-design-presentation).</span>
...
##### Fuente: [Discovering the Domain Architecture](https://www.microsoftpressstore.com/articles/article.aspx?p=2248811&seqNum=3).
...
- **Bif Ball of Mud:** Monolito o [Legacy Code](https://es.wikipedia.org/wiki/C%C3%B3digo_heredado) no disgregado encapsulado/aislado en un *Bounded Context* compartiendo sus servicios. Controlando su crecimiento y aconsejando extraerlo en nuevos *Bounded Context* generando nuevos *Context Maps* entre los existentes. 
...
- **Published Language (PL):** Lenguaje común para la traducción entre los modelos que van a interaccionar. Por ejemplo [JSON](https://en.wikipedia.org/wiki/JSON) o [XML](https://en.wikipedia.org/wiki/XML). Normalmente asociado a OHS. La ventaja de utilizar servicios REST es que en cada petición puede especificarse el PL configurando el *content type* deseado. 
...
##### Fuente: [Strategic Domain Driven Design with Context Mapping](https://www.infoq.com/articles/ddd-contextmapping).

En dicha capa se implementan las *interfaces* expuestas como *servicios de dominio*. 

Tanto el *Adaptor* como el *Translator* de cada entidad del modelo que deseamos traducir o compatibilizar. 

Empleando [Facade](https://en.wikipedia.org/wiki/Facade_pattern) y mediante *HttpClient*: 

<br><center><img src="/wp-content/uploads/acl.png"  width="600"/></center>
##### Fuente: [Domain-Driven Design](https://dzone.com/refcardz/getting-started-domain-driven).
...
Hasta aquí abordo hasta el capítulo 5 de *Implementing Domain-Driven Design* de Vaughn Vernon. 
El resto se centra en los aspectos tácticos de DDD. 
